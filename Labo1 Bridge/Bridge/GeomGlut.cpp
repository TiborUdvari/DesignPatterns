...
void GeomGlut::initGraphicsWin( unsigned int pixelWinX, double xMin, double xMax, double yMin, double yMax )
{
  if(xMax-xMin<=0)
    {
        std::cout << "xMax-xMin cannot be < or = to 0" << std::endl << std::endl;
        return;
    }
  ...
  std::cout << "System coordinate min(" << xMin << ", " << yMin << ") max(" << xMax << ", " << yMax << " give for " << winFuncPixels.x << " pixels in X, a Y pixel number of " << winFuncPixels.y << std::endl << std::endl;

  std::cout << "pixelWin(" << winFuncPixels.x << ", " << winFuncPixels.y << ")...";
  ...
  glutCreateWindow("Algorithmes numeriques: labo1");
