...
GeomGlut* GeomGlut::singleton=NULL;//initialisation of the singleton instance
GeomGlut* GeomGlut::getInstance()
{
    if(singleton==NULL)
    {
        singleton=new GeomGlut();
    }
    return singleton;
}
...
void GeomGlut::initGraphicsWin( unsigned int pixelWinX, double xMin, double xMax, double yMin, double yMax )
{
    ...
    GeomGlut::getInstance()->setWinPixels( winFuncPixels.x, winFuncPixels.y );
    ...
    glutCreateWindow("Design Patterns Labo1 Singleton");
...
void Reshape(int w, int h)
{
    ...
    GeomGlut::getInstance()->setWinPixels( w, h );

    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();

    float xRatio = (float)w/(float)GeomGlut::getInstance()->xWinFunc();
    float yRatio = (float)h/(float)GeomGlut::getInstance()->yWinFunc();

    // Volume de clipping : (left, right, bottom, top, near, far)
    glOrtho(GeomGlut::getInstance()->xMin()*xRatio, GeomGlut::getInstance()->xMax()*xRatio, GeomGlut::getInstance()->yMin()*yRatio, GeomGlut::getInstance()->yMax()*yRatio, -2.0f, 2.0f);
